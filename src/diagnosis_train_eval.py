...
def train_model(model, train_loader, val_loader, optimizer, loss_fn, device,
    ...
    for epoch in range(1, epochs+1):
        ...
        print("cm", trn_metrics['conf_matrix'], val_metrics['conf_matrix'])
...
# ── Multi-image variants (dataset returns imgs, mask, label, patient_id) ─────

def train_one_epoch_multi(model, loader, optimizer, loss_fn, device):
    model.train()
    scaler = GradScaler()
    ...
    for imgs, masks, labels, _ in tqdm(loader, desc="Train"):
        imgs   = imgs.to(device)
        ...
        with autocast(dtype=torch.float16):
            logits = model(imgs, src_key_padding_mask=masks)
            loss   = loss_fn(logits, labels)
...
@torch.no_grad()
def validate_multi(model, loader, loss_fn, device):
    model.eval()

    running_loss = 0
    all_labels, all_preds, all_probs = [], [], []
    correct_files, incorrect_files, all_files = [], [], []

    for imgs, masks, labels, files in tqdm(loader, desc="Val"):
        imgs   = imgs.to(device)
        masks  = masks.to(device)
        labels = labels.long().to(device)
...
def train_model_multi(model, train_loader, val_loader, optimizer, loss_fn, device,
                      epochs=10, patience=5, str_prefix='mBRSET'):
    model.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss, trn_metrics = train_one_epoch_multi(
            model, train_loader, optimizer, loss_fn, device
        )
        val_loss, val_metrics, _, _, _ = validate_multi(
            model, val_loader, loss_fn, device
        )

        print(f"Loss  train={train_loss:.4f}  val={val_loss:.4f}")
        print(f"CM train:\n{trn_metrics['conf_matrix']}")
