...
class UnifiedBackboneMulti(nn.Module):
    ...
    def forward(self, x, src_key_padding_mask=None):
        ...
        feats_all = torch.cat([cls, img_feats], dim=1)  # (B, 1+N, D)

        # Prepend False for the CLS token — it is never masked
        if src_key_padding_mask is not None:
            cls_mask = torch.zeros(B, 1, dtype=torch.bool, device=feats_all.device)
            src_key_padding_mask = torch.cat([cls_mask, src_key_padding_mask], dim=1)  # (B, 1+N)

        enc = self.encoder(feats_all, src_key_padding_mask=src_key_padding_mask)
        cls_out = enc[:, 0, :]
